...
void SamplePlugin::DoOperation1( const StringList& in_rFilenameList )
{
	...
	ISourceControlProgress* pProgress = m_pSourceControlUtilities->GetProgress();

	// Show the progress dialog
	pProgress->BeginOperation();
	...
	pProgress->AddLogMessage(AK::Wwise::ISourceControlProgress::Severity_Info, csMessage );

	// Wait for the user to press 'Ok'
	pProgress->EndOperation();
}
...
ISourceControl::OperationResult SamplePlugin::Connect(const IConnectParameterMap& parameterMap)
{
	return OperationResult_NotImplemented;
}

ISourceControl::OperationResult SamplePlugin::Disconnect()
{
	return OperationResult_NotImplemented;
}
