	...
	LPCTSTR in_pszErrorNameConflict )
{
	...
	for (const auto& csFilename : in_rFilenameList)
	{
		...
		if( PathFileExists(szNewPath) )
		{
			...
			in_pUtilities->GetProgress()->AddLogMessage( AK::Wwise::ISourceControlProgress::Severity_Error, csMsg );
...
bool SourceControlHelpers::WaitWithPaintLoop(HANDLE hEvent)
{
	...
	while(1)
	{
		...
		while(PeekMessage(&msg,NULL,NULL,NULL,PM_REMOVE|PM_QS_PAINT))
		{
