...
void SubversionSourceControl::GetPluginInfo( PluginInfo& out_rPluginInfo )
{
	...
	CString csName;
	csName = _T("Subversion (deprecated)");
...
void SubversionSourceControl::Add(const StringListView& in_rFilenameList, bool in_bShowUI)
{
	...
	if (in_bShowUI)
		m_pUtilities->GetProgress()->EndOperation( bError );
}
...
ISourceControl::OperationResult SubversionSourceControl::PreCreateOrModify( const StringList& in_rFilenameList, CreateOrModifyOperation in_eOperation, bool& out_rContinue )
{
	...
	if ( in_eOperation & CreateOrModifyOperation_Create )
	{
		...
		if ( !m_filesToAdd.empty() )
		{
			...
			if( pProgress && pProgress->IsSilent() ) 
			{
				m_bOnNewFilesAdd = pProgress->IsAutoAdd();
			}
			else
			{
			...
			case SVNErrorDestination_Progress:
				{
					...
					while ( pChildError )
					{
						m_pUtilities->GetProgress()->AddLogMessage( AK::Wwise::ISourceControlProgress::Severity_Error, CString( pChildError->message ) );
...
svn_error_t* SubversionSourceControl::CancelCallback( AK::Wwise::ISourceControlUtilities* in_pUtilities )
{
	...
	ISourceControlProgress* pProgress = in_pUtilities->GetProgress();
...
void SubversionSourceControl::NotifyCallback( SubversionSourceControl* in_pSourceControl, const svn_wc_notify_t* in_pNotifyInfo, apr_pool_t* in_pPool )
{
	...
	if ( in_pSourceControl && in_pNotifyInfo )
	{
		...
		AK::Wwise::ISourceControlProgress::Severity notifySeverity = AK::Wwise::ISourceControlProgress::Severity_Info;
		...
		switch( in_pNotifyInfo->action )
		{
			...
			case svn_wc_notify_failed_revert:
				notifySeverity = AK::Wwise::ISourceControlProgress::Severity_Error;
		...
		if ( !csNotifyText.IsEmpty() )
			in_pSourceControl->m_pUtilities->GetProgress()->AddLogMessage( notifySeverity, csNotifyText );
...
svn_error_t* SubversionSourceControl::ShowLogCallback( AK::Wwise::ISourceControlUtilities* in_pUtilities, apr_hash_t* in_paths, svn_revnum_t in_revision, const char* in_pAuthor, const char* in_pDate, const char* in_pMessage, apr_pool_t* in_pPool )
{
	...
	if ( !pError )
	{
		...
		in_pUtilities->GetProgress()->AddLogMessage( AK::Wwise::ISourceControlProgress::Severity_Info, csLog );
	}
	
	return pError;
}

ISourceControl::OperationResult SubversionSourceControl::Connect(const IConnectParameterMap& parameterMap)
{
	return OperationResult_NotImplemented;
}

ISourceControl::OperationResult SubversionSourceControl::Disconnect()
{
	return OperationResult_NotImplemented;
}
