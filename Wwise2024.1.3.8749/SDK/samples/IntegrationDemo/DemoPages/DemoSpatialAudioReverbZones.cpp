...
static const std::array<const char*, 3> sEmitterEvent =
{
	"Play_Room_Emitter", // default
	"Play_Test_Noise", // noise test
	nullptr // no sound plays.
};

// List of rooms that we will cycle between for playing  a room tone.
static const std::array<AkGameObjectID, 4> sRoomGameObjects = {
	AK_INVALID_GAME_OBJECT, // No t
	ROOM.AsGameObjectID(),
	PATIO.AsGameObjectID(),
	AK::SpatialAudio::kOutdoorRoomID.AsGameObjectID(),
};
	...
	, m_portalOpen(true)
{
	m_szHelp =
		...
		"* <<UG_BUTTON9>> - Cycle the emitter sound between speech, noise, and off.\n"
		"* <<UG_BUTTON10>> - Cycle the room tone playback between outside, patio, room, and off.\n";
}
...
void DemoSpatialAudioReverbZones::InitControls()
{
	...
	AK::SoundEngine::PostEvent(GetEmitterEventName(), EMITTER);
...
void DemoSpatialAudioReverbZones::Init_Rooms()
{
	...
	paramsRoom.RoomGameObj_KeepRegistered = AK::SpatialAudio::kOutdoorRoomID.AsGameObjectID() == GetRoomToneGameObjectID();	// True, if we intend to use the room's game object to post events (see documentation of AkRoomParams::RoomGameObj_KeepRegistered).;
	...
	paramsRoom.RoomGameObj_KeepRegistered = ROOM.AsGameObjectID() == GetRoomToneGameObjectID(); // True, if we intend to use the room's game object to post events (see documentation of AkRoomParams::RoomGameObj_KeepRegistered).;
	...
	paramsRoom.RoomGameObj_KeepRegistered = PATIO_GEOMETRY.AsGameObjectID() == GetRoomToneGameObjectID();	// True, if we intend to use the room's game object to post events (see documentation of AkRoomParams::RoomGameObj_KeepRegistered).
	...
	if (GetRoomToneGameObjectID() != AK_INVALID_GAME_OBJECT)
	{
		AK::SoundEngine::PostEvent("Play_Ambience_Quad", GetRoomToneGameObjectID());
	}
}
...
void DemoSpatialAudioReverbZones::Init_Portals()
{
	...
	m_pPortal->SetPosition((int)(rx + rw - 30), (int)(ry + rh / 2.0f - rh / 10.f));
	m_pPortal->SetDimensions((int)(60), (int)(rh / 5.f));
...
const char* DemoSpatialAudioReverbZones::GetEmitterEventName() const
{
	return sEmitterEvent[m_emitterSoundIndex];
}

AkGameObjectID DemoSpatialAudioReverbZones::GetRoomToneGameObjectID() const
{
	return sRoomGameObjects[m_roomToneIndex];
}
...
bool DemoSpatialAudioReverbZones::Update()
{
	...
	for (it = m_pParentMenu->Input()->Begin(); it != m_pParentMenu->Input()->End(); it++)
	{
		...
		if ((*it).IsButtonTriggered(UG_BUTTON9))
		{
			...
			m_emitterSoundIndex = (m_emitterSoundIndex + 1) % sEmitterEvent.size();
		
			if (GetEmitterEventName() != nullptr)
			{
				AK::SoundEngine::PostEvent(GetEmitterEventName(), EMITTER);
			}
		}

		if ((*it).IsButtonTriggered(UG_BUTTON10))
		{
			if (GetRoomToneGameObjectID() != AK_INVALID_GAME_OBJECT)
				AK::SoundEngine::StopAll(GetRoomToneGameObjectID());

			m_roomToneIndex = (m_roomToneIndex + 1) % sRoomGameObjects.size();
