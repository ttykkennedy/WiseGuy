...
// active page for the devicestatuscallback to use, since we do not provide a userdata for it
static Demo3dAudio* s_pActivePage = nullptr;
void AudioDeviceStatusCallback(AK::IAkGlobalPluginContext* in_pContext, AkUniqueID in_idAudioDeviceShareset, AkUInt32 in_idDeviceID, AK::AkAudioDeviceEvent in_idEvent, AKRESULT in_AkResult)
{
	if (in_idEvent == AK::AkAudioDeviceEvent_Initialization && in_AkResult == AK_Success && s_pActivePage)
	{
		s_pActivePage->OnDeviceChanged(in_idAudioDeviceShareset, in_idDeviceID);
	}
}
...
bool Demo3dAudio::Init()
{
	...
	// Register the metering callback
	...
	RegisterMeteringCallback(uCurrentDeviceID);

	// Register the device status callback
	AKASSERT(s_pActivePage == nullptr);
	s_pActivePage = this;
	AK::SoundEngine::RegisterAudioDeviceStatusCallback(AudioDeviceStatusCallback);
...
void Demo3dAudio::Release()
{
	...
	UnregisterMeteringCallback();
	
	// clear the device status callback
	AK::SoundEngine::UnregisterAudioDeviceStatusCallback();
	AKASSERT(s_pActivePage == this);
	s_pActivePage = nullptr;
...
void Demo3dAudio::OnDeviceChanged(AkUniqueID in_sharesetId, AkUInt32 in_idDeviceID)
{
	UnregisterMeteringCallback();
	RegisterMeteringCallback((((AkUInt64)in_idDeviceID << 32) | in_sharesetId));
}

void Demo3dAudio::RegisterMeteringCallback(AkOutputDeviceID in_idDeviceID)
{
	if (AK_Success == AK::SoundEngine::RegisterOutputDeviceMeteringCallback(in_idDeviceID, DeviceMeteringCallback, AkMeteringFlags::AK_EnableBusMeter_Peak, this))
	{
		m_RegisteredCallbackOutputID = in_idDeviceID;
	}
}

void Demo3dAudio::UnregisterMeteringCallback()
{
	if (m_RegisteredCallbackOutputID != AK_INVALID_OUTPUT_DEVICE_ID)
	{
		AK::SoundEngine::RegisterOutputDeviceMeteringCallback(m_RegisteredCallbackOutputID, nullptr, AkMeteringFlags::AK_NoMetering, this);
		m_RegisteredCallbackOutputID = AK_INVALID_OUTPUT_DEVICE_ID;
	}
}
...
bool Demo3dAudio::Update()
{
	...
	for ( it = m_pParentMenu->Input()->Begin(); it != m_pParentMenu->Input()->End(); it++ )
	{
		...
		else if ((*it).IsButtonTriggered(UG_BUTTON3))
		{
			// Toggle 3D audio
