...
void UniversalInput::AddPointerEvent(const PointerEvent& evt)
{
	AkAutoLock<CAkLock> lock(m_PointerEventsLock);
	// First, search for an existing entry with this ID
	AkUInt64 id = evt.uPointerId;
	for (auto it = m_PointerEvents.begin(); it != m_PointerEvents.end(); ++it)
	{
		if (it->uPointerId == id)
		{
			// Found.
			if (evt.eType == PointerEventType_Released)
			{
				// Pointer has been released, we can remove this entry
				m_PointerEvents.erase(it);
			}
			else
			{
				// Update event
	...
	// Not found; this is a new event. Add if it's active
	if (evt.eType != PointerEventType_Released)
	{
		m_PointerEvents.push_back(evt);
	}
}

static bool PointerIntersects(const PointerEvent& evt, const VirtualButtonLayout& btn)
{
	return
		btn.rx1 <= evt.x && evt.x <= btn.rx2 &&
		btn.ry1 <= evt.y && evt.y <= btn.ry2;
}

void UniversalInput::UpdateStateFromPointerEvents(const VirtualGamepadLayout& layout, AkUInt16 in_nPlayerIndex)
{
	AkAutoLock<CAkLock> lock(m_PointerEventsLock);
	UGBtnState btnState = 0;
	UGStickState stickState[2] = {}; // Virtual Gamepad has no sticks
	// Iterate over all currently active pointer events; if they overlap with one of the buttons, activate the button.
	for (auto it = m_PointerEvents.begin(); it != m_PointerEvents.end(); ++it)
	{
		for (auto bit = layout.buttons.begin(); bit != layout.buttons.end(); ++bit)
		{
			if (PointerIntersects(*it, *bit))
			{
				btnState |= bit->code;
			}
		}
	}
	SetState(in_nPlayerIndex, true, btnState, stickState);
}
