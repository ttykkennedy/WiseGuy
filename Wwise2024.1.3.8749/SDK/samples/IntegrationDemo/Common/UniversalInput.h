...
#define AK_INVALID_UGBUTTON ((UGButton)(-1))
...
//|	BUTTON8	   | C                | Back       | Select     | -          | Z          | Select     |//
//|	BUTTON9	   | Z                | Left Thumb | L3         |            |            |            |//
//|	BUTTON10   | X                | Right Thumb| R3         |            |            |            |//
//|	BUTTON11   |                  |            |            |            |            |            |//
...
/// Descriptor for a single button in a virtual gamepad overlay for a touch-based interface
/// All coordinates are normalized 0..1
struct VirtualButtonLayout
{
	UGButton code = AK_INVALID_UGBUTTON;

	// Surrounding rect box
	float rx1 = 0;
	float ry1 = 0;
	float rx2 = 0;
	float ry2 = 0;
...
struct PointerEvent
{
	AkUInt64 uPointerId; // ID to uniquely identify a gesture across multiple updates
	PointerEventType eType;
	float x; // Normalized from 0 to 1
	float y; // Normalized from 0 to 1
};

/// Descriptor for a virtual gamepad overlay for a touch-based interface
struct VirtualGamepadLayout
{
	std::vector<VirtualButtonLayout> buttons;
	float fHeight = 0; // Height of the Virtual Gamepad overlay, normalized 0..1
};
...
public:
	...
	// Track new incoming pointer event; this function is thread-safe
	void AddPointerEvent(const PointerEvent& evt);

	// Update Gamepad state based on accumulated pointer events; this function is thread-safe
	void UpdateStateFromPointerEvents(
		const VirtualGamepadLayout& layout,
		AkUInt16   in_nPlayerIndex		///< - The player index of the gamepad
	);
...
private:
	...
	/// Container holding the active pointer events; one per pointer ID
	std::vector<PointerEvent> m_PointerEvents;

	CAkLock m_PointerEventsLock;
};
