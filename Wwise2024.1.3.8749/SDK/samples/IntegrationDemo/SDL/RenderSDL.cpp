...
namespace sdlCore
{
	...
	{
		g_pRenderer = SDL_CreateRenderer((SDL_Window*)(in_pParam), -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
		if (g_pRenderer == NULL)
		{
			if (in_szErrorBuffer)
		...
		// High-DPI support; we want the window size in pixels, not logical points
		SDL_GetWindowSizeInPixels((SDL_Window*)(in_pParam), &width, &height);
#else
		// No high-DPI support; please do not use the SDL_WINDOW_ALLOW_HIGHDPI flag for this version of SDL.
