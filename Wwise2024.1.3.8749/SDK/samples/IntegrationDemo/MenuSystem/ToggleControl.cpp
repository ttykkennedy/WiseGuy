...
void ToggleControl::Update( const UniversalGamepad& in_Gamepad )
{
	...
	if ( m_Options.size() > 1 )
	{
		...
		else if ( in_Gamepad.IsButtonTriggered( UG_DPAD_RIGHT ) )
		{
			...
			ControlEvent* pEvent = new ControlEvent;
			pEvent->iPlayerIndex = in_Gamepad.GetPlayerIndex();
			pEvent->iButton = UG_DPAD_RIGHT;
			CallDelegate( pEvent );
			delete pEvent;
		}
		else if (in_Gamepad.IsButtonTriggered(UG_BUTTON1) || in_Gamepad.IsButtonTriggered(UG_BUTTON3) || in_Gamepad.IsButtonTriggered(UG_BUTTON4))
		{
			ControlEvent* pEvent = new ControlEvent;
			pEvent->iPlayerIndex = in_Gamepad.GetPlayerIndex();
			pEvent->iButton = in_Gamepad.GetCurrentState();
			CallDelegate(pEvent);
			delete pEvent;
		}
	}
