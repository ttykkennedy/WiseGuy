...
bool AkWAAPIErrorMessageTranslator::GetInfo(TagInformation* in_pTagList, AkUInt32 in_uCount, AkUInt32& io_uTranslated)
{
	...
	for (AkUInt32 i = 0; i < in_uCount && AKPLATFORM::Elapsed(timeStop, timeStart) < m_msTimeout; ++i)
	{
		if (!in_pTagList[i].m_infoIsParsed)
		{
			...
			{
				AkUInt32 arg = *(AkUInt32*)(in_pTagList[i].m_args);
				const AkUInt32 possibleQueries[] =
				{
					AK::WaapiTranslator::waqlQuery::Global,
					AK::WaapiTranslator::waqlQuery::Event,
					AK::WaapiTranslator::waqlQuery::GameParameter,
					AK::WaapiTranslator::waqlQuery::Trigger,
					AK::WaapiTranslator::waqlQuery::Effect,
					AK::WaapiTranslator::waqlQuery::AudioDevice,
					AK::WaapiTranslator::waqlQuery::DialogueEvent,
					AK::WaapiTranslator::waqlQuery::AcousticTexture
				};	//Other types are either covered by global (all non-hashed Ids) or have their own tag (bank, state, switch)

				for (AkUInt32 j = 0; j < sizeof(possibleQueries) / sizeof(*possibleQueries); j++)
				{
					if (WAAPIGet(s_waqlQuery[possibleQueries[j]], in_pTagList[i], arg))
					{
						++io_uTranslated;
						break;
					}
