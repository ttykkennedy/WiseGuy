...
void CAkIOMemMgr::CheckCacheConsistency( AkMemBlock * in_pMustFindBlock )
{
	...
	while ( it != m_arTaggedBlocks.End() )
	{
		...
		AKASSERT(pThisBlock->cacheID != AK_INVALID_CACHE_ID);	// should not be in the cache!
		if ( pPrevBlock 
			&& ( pPrevBlock->cacheID > pThisBlock->cacheID
				|| ( pPrevBlock->cacheID == pThisBlock->cacheID && pPrevBlock->uPosition < pThisBlock->uPosition )
				|| ( pPrevBlock->cacheID == pThisBlock->cacheID && pPrevBlock->uPosition == pThisBlock->uPosition && pPrevBlock->pData > pThisBlock->pData )
				) )
		{
			AKASSERT( false );
		}
	...
	AKASSERT( bFound || in_pMustFindBlock->cacheID == AK_INVALID_CACHE_ID );
}
#endif
...
{
	AKPLATFORM::OutputDebugMsg( "Index cacheID Position Size\n" );
	AkMemBlocksDictionnary::Iterator it = m_arTaggedBlocks.Begin();
	while ( it != m_arTaggedBlocks.End() )
	{
		AkMemBlock * pMemBlock = (*it);
		AKPLATFORM::OutputDebugMsgV( "%u\t%u\t%lu\t%u\n", (*it), pMemBlock->cacheID, (AkUInt32)pMemBlock->uPosition, pMemBlock->uAvailableSize );
		++it;
	}
}
#endif
...
inline int Compare_CacheID_Position( const AkMemBlock * in_pSorted, AkCacheID in_cacheID, AkUInt64 in_uPosition )
{
	// 1st key: file ID.
	if ( in_cacheID < in_pSorted->cacheID )
		return -1;
	else if ( in_cacheID > in_pSorted->cacheID )
		return 1;
	...
	AKASSERT( in_uPosition == in_pSorted->uPosition && in_cacheID == in_pSorted->cacheID );
	return 0;
}
...
{
	AKASSERT( UseCache() );
	AKASSERT( in_cacheID != AK_INVALID_CACHE_ID 
			&& in_uRequiredAlign >= 1 
	...
	do
	{
		...
		int iCmp = Compare_CacheID_Position( m_arTaggedBlocks[iThis], in_cacheID, in_uPosition );
		if ( 0 == iCmp )
		{
		...
		|| ( in_uPosition > pMemBlock->uPosition + io_uRequestedSize - in_uMinSize ) )	// Perfect match, but is it valid?
	{
		...
		if ( in_cacheID == pCurrentPick->cacheID
		...
		else
		{
			...
			if( iNextBlock < (AkInt32)m_arTaggedBlocks.Length() )
			{
				...
				if ( in_cacheID == pSecondBestBlock->cacheID
...
{
	// 1st key: file ID.
	if ( in_pSearched->cacheID < in_pSorted->cacheID )
		return -1;
	else if ( in_pSearched->cacheID > in_pSorted->cacheID )
		return 1;
	...
	AKASSERT( in_pSearched->uPosition == in_pSorted->uPosition 
			&& in_pSearched->cacheID == in_pSorted->cacheID 
	...
	)
{
	...
	in_pMemBlock->cacheID			= AK_INVALID_CACHE_ID;
...
void CAkIOMemMgr::FlushCache()
{
	if ( m_bUseCache )
	{
		...
		for ( AkMemBlocksDictionnary::Iterator dictIt = m_arTaggedBlocks.Begin(); dictIt != m_arTaggedBlocks.End(); ++ dictIt )
		{
			...
			pMemBlk->cacheID = AK_INVALID_CACHE_ID;
	...
	)
{
	...
	if (in_cacheID == AK_INVALID_CACHE_ID)
	{
	...
	else if (bFound || m_arTaggedBlocks.AddLast(in_pMemBlock) != NULL )
	{
		...
		newBlockData.cacheID				= in_cacheID;
	...
	in_pMemBlock->cacheID			= in_cacheID;
	in_pMemBlock->uPosition			= in_uPosition;
	in_pMemBlock->uAvailableSize	= in_uDataSize;	
	in_pMemBlock->pTransfer			= in_pTransfer;
