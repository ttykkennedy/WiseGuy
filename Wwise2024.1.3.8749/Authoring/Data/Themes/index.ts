...
function sassToJson(inputFile: string, outputFile: string) {
    return new Promise<void>((resolve, reject) => {
        ...
        sass.render({
            ...
            const colorkeyAlias = "Global_TransparentColorKey";
            const colorkey = jsonObject[colorkeyAlias];
            for (const key in jsonObject) {
                const color = jsonObject[key];
                if (color === colorkey && key !== colorkeyAlias) {
                    return reject(`Color ${key} has the same color as ${colorkeyAlias} in ${inputFile}`);
                }
            }
...
async function refreshWwise() {
    try {
        ...
        const session = await waapi.connect('ws://127.0.0.1:8080/waapi');
