			...
			onleaf = function(n)
				...
				if p.global.hasproject(prj) then
					...
					local contents = p.capture(function()
						p.callArray(m.elements.workspaceFileRef, prj, wks)
					end)
					p.outln(contents .. ">")
					p.pop('</FileRef>')
				end
...
---------------------------------------------------------------------------
	...
	function m.workspaceLocation(prj, wks)	-- AUDIOKINETIC mdonais Adding importproject
		local fname = p.filename(prj, ".xcodeproj")
		fname = path.getrelative(wks.location, fname)	-- AUDIOKINETIC mdonais Adding importproject
