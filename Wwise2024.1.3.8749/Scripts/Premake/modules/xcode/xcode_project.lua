...
--
	...
	function xcode.buildprjtree(prj)
		...
		-- AUDIOKINETIC Add the "Resources" folder
		if prj.xcodebuildresources then
			tr.resources = tree.insert(tr, tree.new("Resources")) 
			for _,res in ipairs(prj.xcodebuildresources) do
				local name = path.getname(res)
				node = tree.insert(tr.resources, tree.new(name))
				node.path = res
			end
		end
		-- AUDIOKINETIC end

		-- AUDIOKINETIC the "Frameworks" folder also adds up external dependencies from other workspaces (the ones we will not build ourselves)
		for cfg in project.eachconfig(prj) do
			if cfg.flags and cfg.flags.GlobalSiblings then
				for _, linkobj in ipairs(config.getlinks(cfg, "siblings", "object")) do
					if linkobj.workspace ~= prj.workspace then
						local link = project.getrelative(cfg.project, linkobj.linktarget.fullpath)
						local name = path.getname(link)
						...
						local path = path.getdirectory(link)
						if not table.contains(cfg.libdirs, path) then
							table.insert(cfg.libdirs, path)
						end
					end
				end
			end
	...
	function xcode.addDependency(prj, tr, dep, build)
		...
		local node = tree.insert(xcnode, tree.new(cfg.linktarget.name))		-- AUDIOKINETIC fix
...
---
	...
	function xcode.footer(prj)
		p.pop('\t};')		-- AUDIOKINETIC mdonais
		p.w('\trootObject = 08FB7793FE84155DC02AAC07 /* Project object */;')		-- AUDIOKINETIC mdonais
		p.pop('}')
	end
