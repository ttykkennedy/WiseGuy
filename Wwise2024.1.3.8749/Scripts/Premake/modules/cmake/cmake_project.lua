...
-- AUDIOKINETIC START
function m.currentlistdirpath(prj, abspath)
	local relpath = path.getrelative(prj.location, abspath)
	if (path.isabsolute(relpath)) then
		-- Could not produce a relative path; project may be on a different drive
		return relpath
	end
	-- Path is relative, so we can tell CMake that it is
	return "${CMAKE_CURRENT_LIST_DIR}/" .. relpath
end
-- AUDIOKINETIC END
			...
			if p.fileconfig.hasFileSettings(filecfg) then
				if filecfg.compilebuildoutputs then
					for _, output in ipairs(filecfg.buildoutputs) do
						...
						_p(2, '"%s"', m.currentlistdirpath(prj, output))
						-- AUDIOKINETIC END
					end
				end
			elseif rule then
...
function m.files(prj)
	...
	tree.traverse(tr, {
		onleaf = function(node, depth)
			...
			_p(1, '"%s"', m.currentlistdirpath(prj, node.abspath))
			-- AUDIOKINETIC END
		end
	})
end
...
function m.generate(prj)
	...
	for cfg in project.eachconfig(prj) do
		...
		-- dependencies
		-- AUDIOKINETIC START
		-- To support importproject directive, dependencies must be declared in the workspace file, not the project file
		...
		_p(2, 'ARCHIVE_OUTPUT_DIRECTORY "%s"', m.currentlistdirpath(prj, cfg.buildtarget.directory))
		_p(2, 'LIBRARY_OUTPUT_DIRECTORY "%s"', m.currentlistdirpath(prj, cfg.buildtarget.directory))
		_p(2, 'RUNTIME_OUTPUT_DIRECTORY "%s"', m.currentlistdirpath(prj, cfg.buildtarget.directory))
		-- AUDIOKINETIC END
		_p(1,')')
		
		-- AUDIOKINETIC START
		-- Custom commands
		if type(cfg.cmakevars) == 'table' then
			for name, value in pairs(cfg.cmakevars) do
				_p(1, 'set(%s %s)', name, value)
			end
		end
		-- AUDIOKINETIC END
		...
		if #cfg.externalincludedirs > 0 then
			...
			for _, includedir in ipairs(cfg.externalincludedirs) do
				-- AUDIOKINETIC START
				--_x(2, '"%s"', includedir)
				_x(2, '"%s"', m.currentlistdirpath(prj, includedir))
				-- AUDIOKINETIC END
			end
			_p(1, ')')
		end
		...
		-- AUDIOKINETIC start
		if cfg.includedirsafter == nil then
			cfg.includedirsafter = {}
		end
		-- AUDIOKINETIC end
		...
		if #cfg.libdirs > 0 then
			...
			for _, libdir in ipairs(cfg.libdirs) do
				-- AUDIOKINETIC START
				--_p(2, '"%s"', libdir)
				_x(2, '"%s"', m.currentlistdirpath(prj, libdir))
				-- AUDIOKINETIC END
			end
			_p(1, ')')
		end
		...
		-- AUDIOKINETIC begin
		-- Generator was not escaping build options correctly, causing CMake interpretation failures
		-- all_build_options = ""
		...
		if #cfg.buildoptions > 0 then
			for _, option in ipairs(cfg.buildoptions) do
				_p(1, 'target_compile_options("%s" PRIVATE "%s")', prj.name, option)
			end
		end
		-- AUDIOKINETIC end
