...
function m.generate(wks)
	...
	tree.traverse(tr, {
		onleaf = function(n)
			...
			-- AUDIOKINETIC start
			-- Project path should be relative to the path to the actual workspace we're writing, not the "primary" workspace of the project
			-- This is for cases where you have one workspace importing projects belonging to a different workspace (i.e. WwiseSDK)
			--prjpath = path.getrelative(prj.workspace.location, prjpath)
			prjpath = path.getrelative(wks.location, prjpath)
			-- AUDIOKINETIC end
			...
			if prj.workspace.location == wks.location then
