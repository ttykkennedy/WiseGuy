...
--[[ AUDIOKINETIC Do not set tools value for each configuration
...
--
	...
	function cpp.buildcommand(prj, objext, node)
		...
		-- AUDIOKINETIC
		if (prj.nodepfile) then
			_p('\t$(SILENT) %s $(FORCE_INCLUDE) -o "$@" -c "$<"', flags)
		else
			_p('\t$(SILENT) %s $(FORCE_INCLUDE) -o "$@" -MF "$(@:%%.%s=%%.d)" -c "$<"', flags, objext) -- This line was the original (always generate dependency output file)
		end
		-- AUDIOKINETIC
	end
...
---------------------------------------------------------------------------
	...
	-- AUDIOKINETIC this is now in the makefile
	function make.cppTools(prj)
		local cfg = project.getfirstconfig(prj)
		local toolset = p.tools[_OPTIONS.cc or cfg.toolset or "gcc"]
		if not toolset then
			error("Invalid toolset '" .. cfg.toolset .. "'")
		...
		if tool then
			...
			_p('  CC = %s', tool)  -- AUDIOKINETIC remove indent
			_p('endif' )
		end
		...
		if tool then
			...
			_p('  CXX = %s', tool)  -- AUDIOKINETIC remove indent
			_p('endif' )
		end
		...
		if tool then
			...
			_p('  AR = %s', tool)  -- AUDIOKINETIC remove indent
			_p('endif' )
		end

		tool = toolset.gettoolname(cfg, "rc")
		if tool then
			_p('RESCOMP = %s', tool)  -- AUDIOKINETIC remove indent
		end
		_p('') -- AUDIOKINETIC
	end
