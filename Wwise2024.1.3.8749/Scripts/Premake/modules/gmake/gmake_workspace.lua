...
--
	...
	-- AUDIOKINETIC Subsolution filter
	-- Reimplementation of workspace functions to obey subsolutionfilter of each project
	function make.insubsolution(prj, wks)
	...
	function make.grouptree_filtered(wks)
		if wks.cursubsolution == nil then
			return p.workspace.grouptree(wks)
		end
	
		local tr = p.tree.new()
		for prj in p.workspace.eachproject(wks) do
			prj = p.global.getexportedproject(prj)	-- AUDIOKINETIC
			if prj and make.insubsolution(prj, wks) then
	...
	function make.eachproject_filtered(wks)
		if wks.cursubsolution == nil then
			return p.workspace.eachproject(wks)
		end

		local sorted = {}
		for id, _ in ipairs(wks.projects) do
			local prj = p.workspace.getproject(wks, id)
			if make.insubsolution(prj, wks) then
...
--
	...
	function make.configmap(wks)
		for cfg in p.workspace.eachconfig(wks) do
			...
			for prj in make.eachproject_filtered(wks) do
				if p.global.hasproject(prj) then	-- AUDIOKINETIC
					...
					prj.wconfigmap = oriwconfigmap -- AUDIOKINETIC added workspace-based config maps
				end -- AUDIOKINETIC
			end
			_p('endif')
		end
...
--
	...
	function make.projects(wks)
		-- AUDIOKINETIC all projects should be sorted
		local projects = {}
		for prj in make.eachproject_filtered(wks) do
			table.insert(projects, prj.name)
		end
		table.sort(projects)
		_p('PROJECTS := %s', table.concat(p.esc(projects), " "))
		_p('')
	end
...
--
	...
	function make.workspacePhonyRule(wks)
		local groups = {}
		local tr = make.grouptree_filtered(wks)
		tree.traverse(tr, {
			onbranch = function(n)
