...
--
	...
	function make.getmakefilename(this, searchprjs)
		 -- AUDIOKINETIC mdonais We might not want the makefile to be generated as Makefile.
		if this.flags and this.flags.DisallowMakefile then
			return ".make"
		end
		local count = 0
...
--
	...
	function make.mkdir(dirname)
		...
		_p('else')
		_p('\t-$(SILENT) mkdir $(subst /,\\\\,%s)', dirname) -- AUDIOKINETIC - Use the dash ("-") to keep going on failure, which works better than "if not exists" in multiproc builds
		_p('endif')
	end
...
---------------------------------------------------------------------------
	...
	function make.phonyRules(prj)
		_p('.PHONY: invalidconfig clean prebuild prelink') -- AUDIOKINETIC: Add invalidconfig to fix clean on wrong config
		_p('')
	end
	...
	-- AUDIOKINETIC BEGIN: Add invalidconfig to fix clean on wrong config
	function make.invalidConfigRules(prj)
		_p('invalidconfig:')
		_p('\t@echo Invalid config')
		_p('')
	end
	-- AUDIOKINETIC END
	...
	function make.shellType()
		_p('SHELLTYPE := posix')
		-- AUDIOKINETIC: detect default sh.exe
		-- 1) Exact sh.exe: Using the default sh that can interpret msdos shell
		-- 2) Non-exact sh.exe: Assume the custom sh.exe is POSIX compliant
		_p('ifeq (sh.exe,$(SHELL))')
		_p('\tSHELLTYPE := msdos')
		_p('endif')
		_p('')
	end
	...
	function make.target(cfg)
		_x('  PREMAKE4_BUILDTARGET_BASENAME = %s', cfg.buildtarget.basename ) -- AUDIOKINETIC
