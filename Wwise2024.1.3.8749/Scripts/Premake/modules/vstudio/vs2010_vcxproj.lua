...
--
	...
	m.elements.project = function(prj)
		return {
			...
			m.files,
--			m.projectReferences, -- AUDIOKINETIC: omit auto project references, this screws up aksoundenginedll
...
-- AUDIOKINETIC	if p.config.hasFile(cfg, path.isresourcefile) then
			local contents = p.capture(function ()
				p.push()
	...
	m.elements.link = function(cfg, explicit)
		if cfg.kind == p.STATICLIB then
			...
			return {
				...
				m.importLibrary,
				m.entryPointSymbol,
				m.applicationDataDir, -- AUDIOKINETIC
				m.generateMapFile,
				...
				for prop in p.rule.eachProperty(rule) do
					...
					if value ~= nil then
						...
						if value ~= nil and #value > 0 then
							m.element(cfg, prop.name, nil, '%s', value)
						end
					end
				end
				p.pop()
...
--
	...
	function m.assemblyReferences(prj)
		...
		if #refs > 0 then
			...
			p.pop('</ItemGroup>')
	-- AUDIOKINETIC mdonais: managed projects want the links as references too (based on vs2005_csproj)
	--		end
		elseif cfg.clr and cfg.clr ~= p.OFF then
			if #prj.references > 0 then
				p.push('<ItemGroup>')
				for _, linkname in ipairs(prj.references) do
					p.x('<Reference Include= "%s" />', linkname)
				end
				p.pop('</ItemGroup>')
			end
	-- AUDIOKINETIC End
	...
	function m.generatedFile(cfg, file)
		if file.generated then
			...
			m.element(cfg, "AutoGen", nil, 'true')
			m.element(cfg, "DependentUpon", nil, path)
		end
	end
						...
						for cfg in project.eachconfig(prj) do
							...
							if fcfg and fcfg[fld.name] then
								...
								if value and #value > 0 then
									m.element(cfg, prop.name, m.configPair(cfg), '%s', value)
								end
							end
						end
					end
					-- AUDIOKINETIC mdonais Added excludedFromBuild for rules too
					for cfg in project.eachconfig(prj) do
						local fcfg = fileconfig.getconfig(file, cfg)
						local fileCfgFunc = {
							m.excludedFromBuild
...
---------------------------------------------------------------------------
	...
	function m.additionalIncludeDirectories(cfg, includedirs)
		if #includedirs > 0 then
			...
			if #dirs > 0 then
				m.element(cfg, "AdditionalIncludeDirectories", nil, "%s;%%(AdditionalIncludeDirectories)", table.concat(dirs, ";"))
			end
		end
	end
	...
	function m.additionalLibraryDirectories(cfg)
		if #cfg.libdirs > 0 then
			...
			m.element(cfg, "AdditionalLibraryDirectories", nil, "%s;%%(AdditionalLibraryDirectories)", dirs)
		end
	end
	...
	function m.additionalManifestFiles(cfg)
		...
		if #manifests > 0 then
			m.element(cfg, "AdditionalManifestFiles", nil, "%s;%%(AdditionalManifestFiles)", table.concat(manifests, ";"))
		end
	end

--	AUDIOKINETIC
	function m.applicationDataDir(cfg)
		if cfg.applicationdatadir ~= nil then
			m.element(cfg, "ApplicationDataDir", nil, "%s", project.getrelative(cfg.project, cfg.applicationdatadir))
		end
	end
--	AUDIOKINETIC end
	...
	function m.additionalUsingDirectories(cfg)
		if #cfg.usingdirs > 0 then
			...
			if #dirs > 0 then
				m.element(cfg, "AdditionalUsingDirectories", nil, "%s;%%(AdditionalUsingDirectories)", table.concat(dirs, ";"))
			end
		end
	end
	...
	function m.largeAddressAware(cfg)
		if (cfg.largeaddressaware == true) then
			m.element(cfg, "LargeAddressAware", nil, 'true')
		end
	end
	...
	function m.structMemberAlignment(cfg)
		...
		if value then
			m.element(cfg, "StructMemberAlignment", nil, value)
		end
	end
	...
	function m.additionalCompileOptions(cfg, condition)
		...
		if #opts > 0 then
			...
			m.element(cfg, "AdditionalOptions", condition, '%s %%(AdditionalOptions)', opts)
		end
	end
	...
	function m.additionalLinkOptions(cfg)
		if #cfg.linkoptions > 0 then
			...
			m.element(cfg, "AdditionalOptions", nil, "%s %%(AdditionalOptions)", opts)
		end
	end
	...
	function m.compileAsManaged(fcfg, condition)
		if fcfg.clr and fcfg ~= p.OFF then
			m.element(fcfg, "CompileAsManaged", condition, "true")
		end
	end
	...
	function m.basicRuntimeChecks(cfg, condition)
		...
		-- AUDIOKINETIC Add Prop sheet support for VS
		if prjcfg and (not config.isOptimizedBuild(prjcfg)) and cfg.rtti == p.ON then
			if cfg.clr == p.OFF then
				m.element(cfg, "BasicRuntimeChecks", condition, "EnableFastChecks")
			end
			if cfg.flags.ExtraWarnings then
				m.element(cfg, "SmallerTypeCheck", nil, "true")
			end
		elseif filecfg and (not config.isOptimizedBuild(filecfg)) and cfg.rtti == p.ON then
			if cfg.clr == p.OFF then
				m.element(cfg, "BasicRuntimeChecks", condition, "EnableFastChecks")
			end
			if cfg.flags.ExtraWarnings then
	...
	function m.buildAdditionalInputs(fcfg, condition)
		if fcfg.buildinputs and #fcfg.buildinputs > 0 then
			...
			m.element(fcfg, "AdditionalInputs", condition, '%s', table.concat(inputs, ";"))
		end
	end
	...
	function m.buildLog(cfg)
		if cfg.buildlog and #cfg.buildlog > 0 then
			p.push('<BuildLog>')
			m.element(cfg, "Path", nil, "%s", vstudio.path(cfg, cfg.buildlog))
			p.pop('</BuildLog>')
		end
	end
	...
	function m.buildMessage(fcfg, condition)
		if fcfg.buildmessage then
			m.element(fcfg, "Message", condition, '%s', fcfg.buildmessage)
		end
	end
	...
	function m.linkObjects(fcfg, condition)
		if fcfg.linkbuildoutputs ~= nil then
			m.element(fcfg, "LinkObjects", condition, tostring(fcfg.linkbuildoutputs))
		end
	end
	...
	function m.characterSet(cfg)
		if not vstudio.isMakefile(cfg) then
			...
			m.element(cfg, "CharacterSet", nil, charactersets[cfg.characterset])
		end
	end
	...
	function m.wholeProgramOptimization(cfg)
		if cfg.flags.LinkTimeOptimization then
			m.element(cfg, "WholeProgramOptimization", nil, "true")
		end
	end
	...
	function m.clrSupport(cfg)
		...
		if value then
			m.element(cfg, "CLRSupport", nil, value)
		end
	end
	...
	function m.configurationType(cfg)
		...
		m.element(cfg, "ConfigurationType", nil, types[cfg.kind])
	end
	...
	function m.culture(cfg)
		...
		if value then
			m.element(cfg, "Culture", nil, "0x%04x", tostring(value))
		end
	end
	...
	function m.debugInformationFormat(cfg)
		...
		if (cfg.symbols == p.ON) or (cfg.symbols == "FastLink") or (cfg.symbols == "Full") then
			...
			m.element(cfg, "DebugInformationFormat", nil, value)
	...
	function m.enableDpiAwareness(cfg)
		...
		if value then
			m.element(cfg, "EnableDpiAwareness", nil, value)
		end
	end
	...
	function m.enableEnhancedInstructionSet(cfg, condition)
		...
		if v then
			m.element(cfg, 'EnableEnhancedInstructionSet', condition, v)
		end
	end
	...
	function m.entryPointSymbol(cfg)
		if cfg.entrypoint then
			m.element(cfg, "EntryPointSymbol", nil, cfg.entrypoint)
		end
	end


	function m.exceptionHandling(cfg, condition)
		if cfg.exceptionhandling == p.OFF or cfg.exceptionhandling == "OffNoDefine" then	-- AUDIOKINETIC
			m.element(cfg, "ExceptionHandling", condition, "false")
		elseif cfg.exceptionhandling == "SEH" then
			m.element(cfg, "ExceptionHandling", condition, "Async")
		elseif cfg.exceptionhandling == "On" then
			m.element(cfg, "ExceptionHandling", condition, "Sync")
		elseif cfg.exceptionhandling == "CThrow" then
			m.element(cfg, "ExceptionHandling", condition, "SyncCThrow")
		end
	end
	...
	function m.excludedFromBuild(filecfg, condition)
		if not filecfg or filecfg.flags.ExcludeFromBuild then
			m.element(filecfg, "ExcludedFromBuild", condition, "true")
		end
	end
	...
	function m.exceptionHandlingSEH(filecfg, condition)
		if not filecfg or filecfg.exceptionhandling == "SEH" then
			m.element(filecfg, "UseSafeExceptionHandlers", condition, "true")
		end
	end
	...
	function m.extensionsToDeleteOnClean(cfg)
		if #cfg.cleanextensions > 0 then
			...
			m.element(cfg, "ExtensionsToDeleteOnClean", nil, value .. "$(ExtensionsToDeleteOnClean)")
		end
	end


	function m.fileType(cfg, file)
		m.element(cfg, "FileType", nil, "Document")
	end
	...
	function m.floatingPointExceptions(cfg)
		if cfg.floatingpointexceptions ~= nil then
			if cfg.floatingpointexceptions then
				m.element(cfg, "FloatingPointExceptions", nil, "true")
			else
				m.element(cfg, "FloatingPointExceptions", nil, "false")
			end
		end
	end
	...
	function m.inlineFunctionExpansion(cfg)
		if cfg.inlining then
			...
			m.element(cfg, "InlineFunctionExpansion", nil, types[cfg.inlining])
		end
	end
	...
	function m.forceIncludes(cfg, condition)
		if #cfg.forceincludes > 0 then
			...
			if #includes > 0 then
				m.element(cfg, "ForcedIncludeFiles", condition, table.concat(includes, ';'))
			end
		end
		if #cfg.forceusings > 0 then
			local usings = vstudio.path(cfg, cfg.forceusings)
			if #usings > 0 then
				m.element(cfg, "ForcedUsingFiles", condition, table.concat(usings, ';'))
			end
		end
	end
	...
	function m.functionLevelLinking(cfg)
		if cfg.functionlevellinking ~= nil then
			if cfg.functionlevellinking then
				m.element(cfg, "FunctionLevelLinking", nil, "true")
			else
				m.element(cfg, "FunctionLevelLinking", nil, "false")
			end
	...
	function m.generateDebugInformation(cfg)
		...
		if value then
			m.element(cfg, "GenerateDebugInformation", nil, value)
		end
	end
	...
	function m.generateManifest(cfg)
		if cfg.flags.NoManifest then
			m.element(cfg, "GenerateManifest", nil, "false")
		end
	end
	...
	function m.ignoreDefaultLibraries(cfg)
		if #cfg.ignoredefaultlibraries > 0 then
			...
			m.element(cfg, "IgnoreSpecificDefaultLibraries", condition, table.concat(ignored, ';'))
		end
	end
	...
	function m.ignoreWarnDuplicateFilename(prj)
		...
		if _ACTION > "vs2012" then
			m.element(prj, "IgnoreWarnCompileDuplicatedFilename", nil, "true")
		end
	end
	...
	function m.ignoreImportLibrary(cfg)
		if cfg.kind == p.SHAREDLIB and cfg.flags.NoImportLib then
			m.element(cfg, "IgnoreImportLibrary", nil, "true")
		end
	end
	...
	function m.importLibrary(cfg)
		--AUDIOKINETIC *Really* prevent the generation of an import lib.  Seems the original premake code was buggy.				
		if cfg.kind == p.SHAREDLIB and not cfg.flags.NoImportLib then
			m.element(cfg, "ImportLibrary", nil, "%s", path.translate(cfg.linktarget.relpath))
		end
	end
	...
	function m.intDir(cfg)
		...
		m.element(cfg, "IntDir", nil, "%s\\", objdir)
	end
	...
	function m.intrinsicFunctions(cfg)
		if cfg.intrinsics ~= nil then
			if cfg.intrinsics then
				m.element(cfg, "IntrinsicFunctions", nil, "true")
			else
				m.element(cfg, "IntrinsicFunctions", nil, "false")
			end
		elseif config.isOptimizedBuild(cfg) then
			m.element(cfg, "IntrinsicFunctions", nil, "true")
		end
	...
	function m.keyword(prj)
		...
		if isWin then
			if isMakefile then
				m.element(prj, "Keyword", nil, "MakeFileProj")
			else
				if isManaged or m.isClrMixed(prj) then
					m.targetFramework(prj)
				end

				-- AUDIOKINETIC added MFCDLLProj
				if prj.flags and prj.flags.MFCDLLProj then
					m.element(prj, "Keyword", nil, "MFCDLLProj")
				elseif isManaged then
				-- AUDIOKINETIC end
					m.element(prj, "Keyword", nil, "ManagedCProj")
				else
					m.element(prj, "Keyword", nil, "Win32Proj")
				end
				m.element(prj, "RootNamespace", nil, "%s", prj.name)
			end
		end
	end
	...
	function m.libraryPath(cfg)
		...
		if #dirs > 0 then
			m.element(cfg, "LibraryPath", nil, "%s;$(LibraryPath)", table.concat(dirs, ";"))
		end
	end
	...
	function m.linkIncremental(cfg)
		if cfg.kind ~= p.STATICLIB then
			m.element(cfg, "LinkIncremental", nil, "%s", tostring(config.canLinkIncremental(cfg)))
		end
	end
	...
	function m.linkLibraryDependencies(cfg, explicit)
		...
		if explicit then
			...
			m.element(cfg, "LinkLibraryDependencies", nil, "false")
			p.pop('</ProjectReference>')
		end
	end
	...
	function m.minimalRebuild(cfg)
		...
		then
			m.element(cfg, "MinimalRebuild", nil, "false")
		end
	end
	...
	function m.moduleDefinitionFile(cfg)
		...
		if df then
			m.element(cfg, "ModuleDefinitionFile", nil, "%s", df)
		end
	end
	...
	function m.multiProcessorCompilation(cfg)
		if cfg.flags.MultiProcessorCompile then
			m.element(cfg, "MultiProcessorCompilation", nil, "true")
		end
	end
	...
	function m.nmakeIncludeDirs(cfg)
		if cfg.kind ~= p.NONE and #cfg.includedirs > 0 then
			...
			if #dirs > 0 then
				m.element(cfg, "NMakeIncludeSearchPath", nil, "%s", table.concat(dirs, ";"))
			end
		end
	end
	...
	function m.nmakeOutput(cfg)
		m.element(cfg, "NMakeOutput", nil, "$(OutDir)%s", cfg.buildtarget.name)
	end
	...
	function m.nmakePreprocessorDefinitions(cfg)
		if cfg.kind ~= p.NONE and #cfg.defines > 0 then
			...
			m.element(cfg, 'NMakePreprocessorDefinitions', nil, defines)
		end
	end
	...
	function m.objectFileName(fcfg)
		if fcfg.objname ~= fcfg.basename then
			m.element(fcfg, "ObjectFileName", m.configPair(fcfg.config), "$(IntDir)\\%s.obj", fcfg.objname)
		end
	end
	...
	function m.omitDefaultLib(cfg)
		if cfg.flags.OmitDefaultLibrary then
			m.element(cfg, "OmitDefaultLibName", nil, "true")
		end
	end
	...
	function m.omitFramePointers(cfg)
		...
		if value then
			m.element(cfg,"OmitFramePointers", nil, value)
		end
	end
	...
	function m.optimizeReferences(cfg)
		if config.isOptimizedBuild(cfg) then
			m.element(cfg, "EnableCOMDATFolding", nil, "true")
			m.element(cfg, "OptimizeReferences", nil, "true")
		end
	end
	...
	function m.optimization(cfg, condition)
		...
		if value or not condition then
			m.element(cfg, 'Optimization', condition, value or "Disabled")
		end
	end
	...
	function m.outDir(cfg)
		...
		m.element(cfg, "OutDir", nil, "%s\\", outdir)
	end

	function m.outputFile(cfg)
		-- AUDIOKINETIC
		m.element(cfg, "OutputFile", nil, "$(OutDir)%s", cfg.buildtarget.name)
	end
	...
	function m.executablePath(cfg)
		...
		if #dirs > 0 then
			...
			m.element(cfg, "ExecutablePath", nil, "%s;$(ExecutablePath)", table.concat(dirs, ";"))
		end
	end
	...
	function m.precompiledHeader(cfg, condition)
		...
		if filecfg then
			if prjcfg.pchsource == filecfg.abspath and not prjcfg.flags.NoPCH then
				m.element(cfg, 'PrecompiledHeader', condition, 'Create')
			elseif filecfg.flags.NoPCH then
				m.element(cfg, 'PrecompiledHeader', condition, 'NotUsing')
			end
		else
			if not prjcfg.flags.NoPCH and prjcfg.pchheader then
				m.element(cfg, "PrecompiledHeader", nil, "Use")
				m.precompiledHeaderFile(path.getbasename(prjcfg.pchheader), prjcfg)
			else
				m.element(cfg, "PrecompiledHeader", nil, "NotUsing")
			end
		end
	end
	...
	function m.preprocessorDefinitions(cfg, defines, escapeQuotes, condition)
		if #defines > 0 then
			...
			m.element(cfg, 'PreprocessorDefinitions', condition, defines)
		end
	end
	...
	function m.undefinePreprocessorDefinitions(cfg, undefines, escapeQuotes, condition)
		if #undefines > 0 then
			...
			m.element(cfg, 'UndefinePreprocessorDefinitions', condition, undefines)
		end
	end
	...
	-- AUDIOKINETIC	if prj.name ~= prj.filename then
			m.element(prj, "ProjectName", nil, "%s", prj.name)
	-- AUDIOKINETIC	end
	end
	...
	function m.defaultLanguage(prj)
		if _ACTION >= "vs2012" then
			m.element(prj, "DefaultLanguage", nil, "%s", "en-US")
		end
	end
	-- AUDIOKINETIC End
	...
	-- AUDIOKINETIC Add Prop sheet support for VS
	function m.propertySheets(cfg, prj)
		...
		if #cfg.vs_propsheet > 0 then
			for _, propsheet in ipairs(cfg.vs_propsheet) do
				p.w('<Import Project="' .. premake.esc(path.getrelative(prj.location, propsheet)) .. '" />')
			end
		end
		-- AUDIOKINETIC End
		p.pop('</ImportGroup>')
	...
	function m.propertySheetGroup(prj)
		for cfg in project.eachconfig(prj) do
			m.propertySheets(cfg, prj)		-- AUDIOKINETIC
		end
	end


	function m.referenceCopyLocalSatelliteAssemblies(prj, ref)
		m.element(prj, "CopyLocalSatelliteAssemblies", nil, "false")
	end


	function m.referenceLinkLibraryDependencies(prj, ref)
		m.element(prj, "LinkLibraryDependencies", nil, "true")
	end


	function m.referenceOutputAssembly(prj, ref)
		m.element(prj, "ReferenceOutputAssembly", nil, "true")
	end


	function m.referencePrivate(prj, ref)
		m.element(prj, "Private", nil, "true")
	end


	function m.referenceProject(prj, ref)
		m.element(prj, "Project", nil, "{%s}", ref.uuid)
	end


	function m.referenceUseLibraryDependences(prj, ref)
		m.element(prj, "UseLibraryDependencyInputs", nil, "false")
	end
	...
	function m.runtimeLibrary(cfg)
		...
		if runtime then
			m.element(cfg, "RuntimeLibrary", nil, runtimes[runtime])
		end
	end

	function m.callingConvention(cfg)
		if cfg.callingconvention then
			m.element(cfg, "CallingConvention", nil, cfg.callingconvention)
		end
	end

	function m.runtimeTypeInfo(cfg, condition)
		if cfg.rtti == p.OFF and cfg.clr == p.OFF then -- AUDIOKINETIC
			m.element(cfg, "RuntimeTypeInfo", condition, "false")
		elseif cfg.rtti == p.ON then
			m.element(cfg, "RuntimeTypeInfo", condition, "true")
		end
	end
	...
	function m.bufferSecurityCheck(cfg)
		...
		if cfg.flags.NoBufferSecurityCheck or (toolVersion and toolVersion:startswith("LLVM-vs")) then
			m.element(cfg, "BufferSecurityCheck", nil, "false")
		end
	end
	...
	function m.stringPooling(cfg)
		if cfg.stringpooling ~= nil then
			if cfg.stringpooling then
				...
				m.element(cfg, "StringPooling", nil, "false")
			end
		elseif config.isOptimizedBuild(cfg) then
			m.element(cfg, "StringPooling", nil, "true")
		end
	...
	function m.subSystem(cfg)
		...
		m.element(cfg, "SubSystem", nil, subsystem)
	end
	...
	function m.targetExt(cfg)
		...
		if ext ~= "" then
			m.element(cfg, "TargetExt", nil, "%s", ext)
		else
			if cfg.removeelements and table.findKeyByValue(cfg.removeelements, "TargetExt") then		-- AUDIOKINETIC mdonais added removeelements to selectively disable elements
				return
			end
			p.w('<TargetExt>')
			p.w('</TargetExt>')
...
--		if cfg.kind == p.STATICLIB and config.hasFile(cfg, path.isresourcefile) then -- AUDIOKINETIC
			local value = targetmachine[cfg.architecture]
			if value ~= nil then
				m.element(cfg, "TargetMachine", nil, '%s', value)
			end
--		end -- AUDIOKINETIC
	end
	...
	function m.targetName(cfg)
		m.element(cfg, "TargetName", nil, "%s%s", cfg.buildtarget.prefix, cfg.buildtarget.basename)
	end
	...
	function m.preferredToolArchitecture(prj)
		if _ACTION >= "vs2013" then
			if prj.preferredtoolarchitecture == p.X86_64 then
				m.element(prj, "PreferredToolArchitecture", nil, 'x64')
			elseif prj.preferredtoolarchitecture == p.X86 then
				m.element(prj, "PreferredToolArchitecture", nil, 'x86')
			end
		else
			if prj.preferredtoolarchitecture == p.X86_64 then
				m.element(prj, "UseNativeEnvironment", nil, 'true')
			end
		end
	end
	...
	function m.vcpkg(prj)
		for cfg in project.eachconfig(prj) do
			if cfg.vcpkg and cfg.vcpkg ~= "Default" then
				if cfg.vcpkg == "On" then
					p.w('<%s %s>%s</%s>', "VcpkgEnabled", m.condition(cfg), 'true', "VcpkgEnabled")
					local runtime = cfg.runtime or iif(config.isDebugBuild(cfg), "Debug", "Release")
					p.w('<%s %s>%s</%s>', "VcpkgConfiguration", m.condition(cfg), runtime, "VcpkgConfiguration")
				else
					p.w('<%s %s>%s</%s>', "VcpkgEnabled", m.condition(cfg), 'false', "VcpkgEnabled")
				end
			end
		end
	end
	...
	function m.treatLinkerWarningAsErrors(cfg)
		if cfg.flags.FatalLinkWarnings then
			...
			m.element(cfg, "Treat" .. el .. "WarningAsErrors", nil, "true")
		end
	end
	...
	function m.treatWChar_tAsBuiltInType(cfg)
		...
		if value then
			m.element(cfg, "TreatWChar_tAsBuiltInType", nil, value)
		end
	end
	...
	function m.treatWarningAsError(cfg)
		if cfg.flags.FatalCompileWarnings and cfg.warnings ~= p.OFF then
			m.element(cfg, "TreatWarningAsError", nil, "true")
		end
	end
	...
	function m.disableSpecificWarnings(cfg, condition)
		if #cfg.disablewarnings > 0 then
			...
			m.element(cfg, 'DisableSpecificWarnings', condition, warnings)
		end
	end
	...
	function m.treatSpecificWarningsAsErrors(cfg, condition)
		if #cfg.fatalwarnings > 0 then
			...
			m.element(cfg, 'TreatSpecificWarningsAsErrors', condition, fatal)
		end
	end
	...
	function m.useDebugLibraries(cfg)
		...
		m.element(cfg, "UseDebugLibraries", nil, tostring(runtime:endswith("Debug")))
	end


	function m.useOfMfc(cfg)
		if cfg.flags.MFC then
			m.element(cfg, "UseOfMfc", nil, iif(cfg.staticruntime == "On", "Static", "Dynamic"))
		end
	end

	function m.useOfAtl(cfg)
		if cfg.atl then
			m.element(cfg, "UseOfATL", nil, iif(cfg.atl == "Off", "false", cfg.atl))
		end
	end
	...
	-- AUDIOKINETIC BEGIN
	local warningLevelMap = {
		Off = "TurnOffAllWarnings",
		Extra = "Level4",
		High = "Level3",
		Level0 = "TurnOffAllWarnings",
		Level1 = "Level1",
		Level2 = "Level2",
		Level3 = "Level3",
		Level4 = "Level4",
	...
	function m.warningLevel(cfg)
		if cfg.warnings then
			m.element(cfg, "WarningLevel", nil, warningLevelMap[cfg.warnings])
		end
	end
	...
	function m.fxCompilePreprocessorDefinition(cfg, condition)
		if cfg.shaderdefines and #cfg.shaderdefines > 0 then
			...
			m.element(cfg, 'PreprocessorDefinitions', condition, shaderdefines)
		end
	end
	...
	function m.fxCompileShaderType(cfg, condition)
		if cfg.shadertype then
			m.element(cfg, "ShaderType", condition, cfg.shadertype)
		end
	end
	...
	function m.fxCompileShaderModel(cfg, condition)
		if cfg.shadermodel then
			m.element(cfg, "ShaderModel", condition, cfg.shadermodel)
		end
	end
	...
	function m.fxCompileShaderEntry(cfg, condition)
		if cfg.shaderentry then
			m.element(cfg, "EntryPointName", condition, cfg.shaderentry)
		end
	end
	...
	function m.fxCompileShaderVariableName(cfg, condition)
		if cfg.shadervariablename then
			m.element(cfg, "VariableName", condition, cfg.shadervariablename)
		end
	end
	...
	function m.fxCompileShaderHeaderOutput(cfg, condition)
		if cfg.shaderheaderfileoutput then
			m.element(cfg, "HeaderFileOutput", condition, cfg.shaderheaderfileoutput)
		end
	end
	...
	function m.fxCompileShaderObjectOutput(cfg, condition)
		if cfg.shaderobjectfileoutput then
			m.element(cfg, "ObjectFileOutput", condition, cfg.shaderobjectfileoutput)
		end
	end
	...
	function m.fxCompileShaderAssembler(cfg, condition)
		if cfg.shaderassembler then
			m.element(cfg, "AssemblerOutput", condition, cfg.shaderassembler)
		end
	end
	...
	function m.fxCompileShaderAssemblerOutput(cfg, condition)
		if cfg.shaderassembleroutput then
			m.element(cfg, "AssemblerOutputFile", condition, cfg.shaderassembleroutput)
		end
	end
...
-- @param cfg
--    The current configuration or project or container to which this element pertains
--    AUDIOKINETIC mdonais added cfg parameter to elements, so we can remove parameters conditionally
-- @param name
--    The element name.
-- @param condition
...
--
	...
	function m.element(cfg, name, condition, value, ...)
		if cfg and cfg.removeelements and table.findKeyByValue(cfg.removeelements, name) then
			return
		end
		local arg = {...}
		if select('#',...) == 0 then
