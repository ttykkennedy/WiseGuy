...
--
	...
	function vstudio.archFromConfig(cfg, win32)
		local isnative = project.isnative(p.global.getexportedproject(cfg.project))	-- AUDIOKINETIC mdonais Adding importproject

		local arch = architecture(cfg.system, cfg.architecture)
		-- AUDIOKINETIC No more Mixed Platforms or Any CPU, just x64
		if not arch then
			arch = "x64"
		end
		-- AUDIOKINETIC end
...
---
	...
	function vstudio.getLinks(cfg, explicit)
		return p.tools.msc.getlinks(cfg, not explicit, true)	-- AUDIOKINETIC mdonais visual studio shouldn't have groups
	end
...
-- AUDIOKINETIC			cfg._needsExplicitLink = ex
			cfg._needsExplicitLink = true	-- AUDIOKINETIC
		end
...
--
	...
	function vstudio.projectfile(prj)
		...
		-- AUDIOKINETIC VS Force file name
		-- Use forced file name if set.
		local fname
...
--
	...
	function vstudio.solutionPlatform(cfg)
		...
		if platform then
			...
			-- AUDIOKINETIC fix Win32 on x86 identifier
...
--					slnarch = "Mixed Platforms"
		...
		if slnarch then
			return iif(slnarch == "x86", "Win32", slnarch) -- iif(slnarch == "x86" and not hasnet, "Win32", slnarch)
		else
			return "x64" -- "Win32"
		end
-- AUDIOKINETIC End
...
-- AUDIOKINETIC No more Mixed Platforms or Any CPU
--		local hasnative = false
...
--			end
		...
		arch = arch or "Win32" -- arch or iif(hasnative, "Win32", "Any CPU")
-- AUDIOKINETIC End
		return arch
	end
