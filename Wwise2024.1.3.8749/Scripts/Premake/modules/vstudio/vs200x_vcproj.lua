	...
	m.elements.configuration = function(cfg)
		if cfg.fake then
			...
			return {
				...
				m.characterSet,
				m.managedExtensions,
				m.inheritedPropertySheets, -- AUDIOKINETIC
			}
		end
	end
...
---
	...
	function m.globals(prj)
		p.push('<Globals>')
		-- AUDIOKINETIC Adding globals to support VSIs
--		p.callArray(m.elements.globals, prj)
		for _, global in ipairs(prj.globals) do
			p.w("<Global %s/>", global)			
		end
		-- AUDIOKINETIC end
		p.pop('</Globals>')
...
---------------------------------------------------------------------------
	...
	function m.additionalDependencies(cfg, toolset)
		...
		if not toolset then
			...
			links = path.translate(toolset.getlinks(cfg, not ex, true))	-- AUDIOKINETIC mdonais visual studio with clang/gcc shouldn't have groups
		end
	...
	function m.applicationDataDir(cfg)
		if cfg.applicationdatadir ~= nil then
			p.x("ApplicationDataDir=%s", project.getrelative(cfg.project, cfg.applicationdatadir))
		end
	end
	-- AUDIOKINETIC end
	...
	function m.enableFunctionLevelLinking(cfg)
		if #cfg.vs_propsheet == 0 then -- AUDIOKINETIC Add Prop sheet support for VS
			local cfg, filecfg = config.normalize(cfg)
			if not filecfg then
	...
	function m.importLibrary(cfg, toolset)
		if cfg.kind == p.SHAREDLIB and not toolset and not cfg.flags.NoImportLib then --AUDIOKINETIC *Really* prevent the generation of an import lib.
	...
	function m.inheritedPropertySheets(cfg)
		if #cfg.vs_propsheet > 0 and _ACTION > "vs2003" then
			local propsheets = "$(VCInstallDir)VCProjectDefaults\\UpgradeFromVC71.vsprops"
			for _,propsheet in ipairs(cfg.vs_propsheet) do
				propsheets = propsheets .. ";" .. propsheet
			end			
			p.w('InheritedPropertySheets="' .. premake.esc(propsheets) .. '"')
		end				

		if cfg.vs_props then						
			for _,prop in ipairs(cfg.vs_props) do
				p.w(prop)					
			end			
		end	
	end
	...
	function m.optimization(cfg)
		...
		if value or not cfg.abspath or #cfg.vs_propsheet == 0 then -- AUDIOKINETIC Add Prop sheet support for VS
...
--				p.w('ProgramDataBaseFileName=""')
				p.w('ProgramDataBaseFileName="$(OutDir)\\%s.pdb"', path.getbasename(cfg.buildtarget.name))
				p.w('DebugInformationFormat="%s"', vc200x.Symbols(cfg))
			end
		end
		-- AUDIOKINETIC end
	end
