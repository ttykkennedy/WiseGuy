...
--
	...
	function dotnetbase.dofile(node, cfg, condition)
		...
		if filecfg then
			...
			-- AUDIOKINETIC mcooper: setting this leads to a <Link /> item for each file, which seems to be very problematic,
			--  and creates a janky folder hierarchy under Wwise/Authoring/Projects...
...
--
	...
	function dotnetbase.assemblyReferences(cfg)
		...
		-- AUDIOKINETIC mdonais: You can add reference by adding them to the references section
		for _, linkname in ipairs(prj.references) do
			_p(2,'<Reference Include= "%s" />', linkname)
		end
		-- AUDIOKINETIC end
	end
	...
	function dotnetbase.projectReferences(prj)
		...
		if #deps > 0 then
			...
			for _, dep in ipairs(deps) do
				-- AUDIOKINETIC mdonais In vs2013, we want to merely have the Reference, not the ProjectReference, because MsBuild hates it otherwise
				local cfg = project.getfirstconfig(dep)
				local refname = cfg.buildtarget.basename or path.getbasename(vstudio.projectfile(dep))
				_x(2,'<Reference Include="%s" />', refname)
--[[
