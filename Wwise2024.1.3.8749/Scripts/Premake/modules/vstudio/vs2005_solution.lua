...
--
	...
	-- AUDIOKINETIC mdonais: add subsolutions and filters
	function sln2005.insubsolution(prj, wks)
		if wks.cursubsolution == nil then
			return true
		end
...
--
	...
	function sln2005.projectdependencies(prj, wks) -- AUDIOKINETIC
		local deps = project.getdependencies(prj, nil, wks) -- AUDIOKINETIC pmilot add wks parameter
		if #deps > 0 then
			p.push('ProjectSection(ProjectDependencies) = postProject')
			for _, dep in ipairs(deps) do
				-- AUDIOKINETIC solution filters in VS		
				if sln2005.insubsolution(dep, wks) then
					for prj in premake.solution.eachproject(wks) do
						if sln2005.insubsolution(prj, wks) and dep.uuid == p.global.getexportedproject(prj).uuid then
...
--
	...
	sln2005.elements.projectConfigurationPlatforms = function(cfg, context)
		return {
			sln2005.activeCfg,
			sln2005.build0,
			-- AUDIOKINETIC set projects to be auto-deployed in build by default (where applicable)
			sln2005.deployProject,
			-- AUDIOKINETIC end
		}
	end
			...
			onleaf = function(n)
				...
				if p.global.hasproject(prj) and sln2005.insubsolution(prj, wks) then -- AUDIOKINETIC mdonais: add subsolutions and filters; Adding importproject
					...
					table.foreachi(sorted, function(cfg)
						...
						prj.wconfigmap = wks.wconfigmap -- AUDIOKINETIC added workspace-based config maps
						context.prj = prj
						...
						prj.wconfigmap = nil -- AUDIOKINETIC added workspace-based config maps
	...
	-- AUDIOKINETIC set projects to be auto-deployed in build by default (where applicable)
	function sln2005.deployProject(cfg, context)
		if not context.excluded and (context.prjCfg.kind == "ConsoleApp" or context.prjCfg.kind == "WindowedApp") then
