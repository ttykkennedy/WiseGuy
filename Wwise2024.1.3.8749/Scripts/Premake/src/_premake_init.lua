...
-----------------------------------------------------------------------------
	...
	-- AUDIOKINETIC: mdonais icon for application
	api.register {
	 ...
	 -- AUDIOKINETIC NX application data dir
	api.register {
	...
	-- AUDIOKINETIC added workspace-based config maps - WARNING: currently hacked to only run in vs2005_solution.lua!
	api.register {
		name = "wconfigmap",
		scope = "workspace",
		kind = "list:keyed:array:string",
...
-- AUDIOKINETIC No support for dependency files in QNX QCC
	api.register {
		name = "nodepfile",
		scope = "project",
		kind = "boolean",
...
-- AUDIOKINETIC file-specific optimization flag in VS
	api.register {
		name = "optimizationperfile",
		scope = "config",
		kind = "list:file",
	}
		
-- AUDIOKINETIC VS property sheets
	api.register {
		name = "vs_propsheet",
		scope = "config",
		kind = "list:file",
...
-- END AUDIOKINETIC
	...
	api.register {
		...
		allowed = {
			...
			"SEH",
			"CThrow",
			"OffNoDefine",		-- AUDIOKINETIC - do not add _HAS_EXCEPTIONS=0 but still off.
		},
	}
	...
	api.register {
		...
		allowed = {
			...
			"C99",                 -- DEPRECATED
			"C11",                 -- DEPRECATED
			"FlatNamespace",		-- AUDIOKINETIC Use flat namespace linking on macOS
			"MFCDLLProj",			-- AUDIOKINETIC added MFCDLLProj
			"NoInvalidOffsetOf",	-- AUDIOKINETIC
			"NoDeprecatedWarning",	-- AUDIOKINETIC
			"OmitUserFiles",		-- AUDIOKINETIC Make sure we don't generate .user files
			"ForceUserFiles",		-- AUDIOKINETIC Make sure we do generate .user files
			"OmitFiltersFiles",		-- AUDIOKINETIC Make sure we don't generate .filters files
			"ForceFiltersFiles",	-- AUDIOKINETIC Make sure we do generate .filters files
			"EmitProjectProperties",-- AUDIOKINETIC For Android: Emit a file containing the project.properties with the target automatically
			"DisableDebugStripping",-- AUDIOKINETIC For Android: A hack to disable the stripping of the debug information
			"DisallowMakefile",		-- AUDIOKINETIC For GMake: Premake5 now creates "Makefile" if there's only one makefile in a folder. Disallow that operation, as it can clash when we are doing multiple platforms
			"NativeDependencyPath",	-- AUDIOKINETIC For VS NX, makes sure to use Visual Studio's paths for Additional Dependencies, otherwise they are shown as -l*
			"GlobalSiblings",		-- AUDIOKINETIC For VS NX, when on the lookup for sibling projects, look them up on all the generated workspaces, not only our current workspace.
		},
		aliases = {
	...
	api.register {
		...
		allowed = {
			...
			"WindowedApp",
			"Utility",
			"Reference", -- AUDIOKINETIC Referenced projects in VS
			"SharedItems",
		},
	}
	...
	-- AUDIOKINETIC PCH header output file
	-- fixes #1390
	api.register {
		name = "pchoutput",
		scope = "config",
	...
	-- AUDIOKINETIC mdonais done with links doing 32049 things. These are managed references, which need to be referenced, but not linked in projects.
	api.register {
		name = "references",
		scope = "config",
		kind = "list:string",
	...
	-- AUDIOKINETIC mdonais remove particular elements in a configuration. Useful in case we are already setting something in a property sheet (right now only supported in vstudio)
	api.register {
		name = "removeelements",
		scope = "config",
		kind = "list:string"
	...
	-- AUDIOKINETIC mdonais project filter: if present, only adds this project to a subsolution if in the filter
	api.register {
		name = "subsolutionfilter",
		scope = "project",
		kind = "list:string",
	}

	-- AUDIOKINETIC mdonais subsolutions: if present, generate more solutions than one at once, and remove anything not in projectsubsolution
	api.register {
		name = "subsolutions",
		scope = "workspace",
		kind = "list:string"
	...
	-- AUDIOKINETIC: Specify the symbol path for the compiler .pdb/.idb
	api.register {
		name = "clsymbolspath",
		scope = "config",
		kind = "path",
	...
	api.register {
		...
		allowed = {
			...
			"windows",
			"tvos",			-- AUDIOKINETIC
			"visionos",		-- AUDIOKINETIC
			"emscripten",	-- AUDIOKINETIC
			"qnx",			-- AUDIOKINETIC
			"android",		-- AUDIOKINETIC
			"openharmony",	-- AUDIOKINETIC
		},
	}
	...
	-- AUDIOKINETIC Visual Studio's VC Packages management. Unless used, you should disable them.
	api.register {
		name = "vcpkg",
		scope = "config",
		kind = "string",
...
-----------------------------------------------------------------------------
	...
	newoption
	{
		...
		allowed = {
			...
			-- AUDIOKINETIC 1:1 with system
			{ "ios",      "Apple iOS" },
			{ "tvos",     "Apple tvOS" },
			{ "visionos", "Apple visionOS" },
			{ "emscripten", "LLVM to Assembly" },
			{ "qnx",      "Blackberry QNX" },
			{ "android",  "Android" },
	...
	-- AUDIOKINETIC added profiler option (this is unused - all the code is in _premake_main
	newoption
	{
		trigger = "profile",
		description = "Enables Premake profiler."
	}
...
-----------------------------------------------------------------------------
	...
	filter { "system:darwin", "action:xcode4", "kind:WindowedApp" }
		targetextension ".app"

	-- AUDIOKINETIC: gmake does not produce proper .app packages
	filter { "system:darwin", "action:gmake", "kind:WindowedApp" }
		targetextension ""
