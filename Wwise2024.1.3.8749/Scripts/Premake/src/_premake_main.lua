...
print("Using Scripts/Premake script")
...
-- AUDIOKINETIC mdonais Verify if the host is the proper version
	if _VERSION == "Lua 5.1" then
		error("** Error: Running through Premake4 host! Please use the latest Premake5 from Github.",0)
	elseif _VERSION ~= "Lua 5.3" then
		print("** Warning: Unknown Lua version: " .. _VERSION .. "! Please update _premake_main if this version is OK to be used!")
	end
	_AK_PREMAKE = true
-- AUDIOKINETIC End

-- AUDIOKINETIC mdonais Embedded profiler
	for i, arg in ipairs(_ARGV) do
		local key, value
		local i = arg:find("=", 1, true)
		if i then
			key = tostring(arg:sub(1, i - 1)):lower()
			value = arg:sub(i + 1)
		else
			key = tostring(arg):lower()
			value = "1"
		end
		if key == "--profile" and value and value ~= "0" then
			require '_premake_profiler'
			premake.profiler.new()
			break
		end
	end
-- AUDIOKINETIC End
...
---
	...
	function m.postAction()
		if p.action.isConfigurable() then
			-- AUDIOKINETIC mdonais show summary of changes
			print("Summary: " .. p.generatedFiles.new .. " new, " .. p.generatedFiles.updated .. " updated, " .. p.generatedFiles.nochange .. " without changes. " ..
				p.generatedFiles.new + p.generatedFiles.updated + p.generatedFiles.nochange .. " total.")

			local duration = math.floor((os.clock() - startTime) * 1000);
			-- Timing is removed because it's the only thing that changes in the logs. Easy enough to reinstate if needed
...
---
-- AUDIOKINETIC Embedded profiler
---

	function m.endProfiler()
		if p.profiler then
			p.profiler.stopAndReport()
		end
	end
