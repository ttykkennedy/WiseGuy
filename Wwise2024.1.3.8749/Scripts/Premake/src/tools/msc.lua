...
--
	...
	msc.shared = {
		...
		warnings = {
			Off = "/W0",
			Level0 = "/W0", -- AUDIOKINETIC
			Level1 = "/W1", -- AUDIOKINETIC
			Level2 = "/W2", -- AUDIOKINETIC
			Level3 = "/W3", -- AUDIOKINETIC
			Level4 = "/W4", -- AUDIOKINETIC
			All = "/Wall", -- AUDIOKINETIC
			High = "/W4",
			Extra = "/W4",
			Everything = "/Wall",
		},
		externalwarnings = {
			Off = "/external:W0",
			Level0 = "/external:W0", -- AUDIOKINETIC
			Level1 = "/external:W1", -- AUDIOKINETIC
			Level2 = "/external:W2", -- AUDIOKINETIC
			Level3 = "/external:W3", -- AUDIOKINETIC
			Level4 = "/external:W4", -- AUDIOKINETIC
			All = "/external:W4", -- AUDIOKINETIC
			Default = "/external:W3",
			High = "/external:W4",
			Extra = "/external:W4",
...
--
	...
	function msc.getlinks(cfg, systemonly, nogroups)
		...
		for i = 1, #system do
			...
			-- AUDIOKINETIC support other toolset even while parsing msc toolset (Can be used in vstudio, while asking to be native)
			local exts = false
			local toolset = config.toolset(cfg)
			if toolset and toolset.getLibraryExtensions then
				exts = toolset.getLibraryExtensions()
			else
				exts = p.tools.msc.getLibraryExtensions()
			end

			if not exts[link:match("[^.]+$")] then
