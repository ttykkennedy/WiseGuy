...
--
	...
	function dotnet.fileinfo(fcfg)
		...
		elseif ext == ".xaml" then
			-- AUDIOKINETIC We have an app project with an App file that must not be an Application Definition, but must be a page.
			if fcfg.buildaction == "Page" then
				info.action = "Page"
			elseif fcfg.buildaction == "Application" or path.getbasename(fname) == "App" then
			-- AUDIOKINETIC End
