	...
	function criteria.new(terms, unprefixed)
		...
		for i, term in ipairs(terms) do
			...
			for _, word in ipairs(words) do
				...
				-- AUDIOKINETIC use absolute paths when a path is detected, as filtering sometimes misfires
				if prefix == "files" and word[1]:match("/") then
					word[1] = path.getabsolute(word[1]):lower()
				end

				if word[4] then
					word[1] = path.wildcards(word[1])
				end
				-- AUDIOKINETIC end
		...
		while (true) do
			if word:startswith("not ") then
				...
				if prefix and i then
					...
					-- AUDIOKINETIC Need to convert wildcards later on
					--if wildcard then
