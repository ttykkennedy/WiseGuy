...
---
	...
	function oven.addGeneratedFiles(wks)
		...
		for prj in p.workspace.eachproject(wks) do
		 	if prj._ ~= nil then	-- AUDIOKINETIC mdonais Adding importproject
			...
			end	-- AUDIOKINETIC mdonais Adding importproject
			
			-- generated files might screw up the object sequences.
			if prj.hasGeneratedFiles and not p.global.isprojectimported(prj) and p.project.isnative(prj) then	-- AUDIOKINETIC mdonais Adding importproject
	...
	function p.project.bake(self)
		-- AUDIOKINETIC mdonais Adding importproject; Be more verbose of what we are baking! Big projects!
		if p.global.isprojectimported(self) then
			if p.global.hasproject(self) then
				verbosef('    Baking %s (%s)... [Imported from %s]', self.name, self.parent.name, p.global.getexportedproject(self).parent.name)
			else
				warnf('Project %s (%s) is imported but never exported.', self.name, self.parent.name)
			end
			return
		end

		verbosef('    Baking %s (%s)...', self.name, self.parent.name)
		-- AUDIOKINETIC End
...
--
	...
	function oven.bakeObjDirs(wks)
		...
		local function getobjdirs(cfg)
			if cfg.processedobjdir then		-- AUDIOKINETIC mdonais Adding importproject
				return nil
			end
			...
			if i then
				...
				cfg.processedobjdir = true	-- AUDIOKINETIC mdonais Adding importproject
				return nil
			end
		...
		for prj in p.workspace.eachproject(wks) do
			if p.global.hasproject(prj) then	-- AUDIOKINETIC mdonais Adding importproject
				for cfg in p.project.eachconfig(p.global.getexportedproject(prj)) do -- AUDIOKINETIC
...
--
	...
	function oven.finishConfig(cfg)
		...
		-- AUDIOKINETIC hack to support undefined architecture in SDK makefile (debug instead of debug_x64).
		-- The problem must be fixed at the root and this hack removed.
		if cfg.platform == nil or cfg.platform == "" then
			cfg.shortname = cfg.buildcfg:lower()
		else
