...
---
	...
	function config.canLink(cfg, target, linkage)
		...
		if type(target) ~= "string" then
			...
			-- AUDIOKINETIC don't try to link managed dlls
			...
			local cfgManaged = project.isdotnet(cfg.project) -- AUDIOKINETIC or (cfg.clr ~= p.OFF)
			local tgtManaged = project.isdotnet(target.project) -- AUDIOKINETIC or (target.clr ~= p.OFF)
...
---
	...
	function config.getlinkinfo(cfg)
		...
		if p.global.isprojectimported(cfg.project) then
			local prj = p.global.getexportedproject(cfg.project)
			return config.getlinkinfo(prj.getconfig(cfg.name, cfg.buildcfg, cfg.platform))
		end
		-- AUDIOKINETIC end
...
--
	...
	function config.getlinks(cfg, kind, part, linkage)
		...
		for i = 1, #cfg.links do
			...
			if not prj and cfg.flags and cfg.flags.GlobalSiblings and p.global.hasproject(link) then	-- AUDIOKINETIC some projects might wish to look for global projects for precise linking options
				prj = p.global.getexportedproject(link)
			else
			...
			if prj and kind ~= "system" then
				...
				prj.wconfigmap = cfg.workspace.wconfigmap				-- AUDIOKINETIC Adding workspace-based maps
			...
			-- Add it to the list, removing and adding back duplicates to the end - AUDIOKINETIC mdonais Allows to set precise dependency order by allowing pushing important things to the end

			if item then
				result = table.filter(result, function(rhs)
					return item ~= rhs
				end)
