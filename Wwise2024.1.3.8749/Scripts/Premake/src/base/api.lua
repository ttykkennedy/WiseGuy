...
---
	...
	function api.container(containerName, parentContainer, extraScopes)
		...
		_G["external" .. containerName] = function(name)
			-- AUDIOKINETIC mdonais Adding external only when it's the first time we encounter a project (it should be)
			local isNew = not api._hasContainer(class, name)
			local c = _G[containerName](name)
			if isNew then
				c.external = true
			elseif c.external ~= true then
				error("A project that was previously declared as non-external is now declared external", 3)
			end
			-- AUDIOKINETIC End
			return c
		end
		
		-- AUDIOKINETIC mdonais Adding importproject so you can import a project created in another workspace
		_G["import" .. containerName] = function(name)
			if not name then
				error("No name while import" .. containerName, 2)
			end
			local origName = name
			local importWksName = string.explode(name, ":")
			if #importWksName > 1 then				-- Either we get the name of the workspace we wish to import
				name = importWksName[2]
				importWksName = importWksName[1]
			else
				importWksName = true				-- ... or we set import to true
			end
	
			local isNew = not api._hasContainer(class, name)

			-- import is a method to import a project to our workspace, it shouldn't change the container.
			if not class.parent then
				error("no class parent while importing " .. name, 3)
				return
			end
			
			local parent = api.scope[class.parent.name]
			if not parent then
			...
			if not isNew then
				local prev = p.container.getChild(parent, class, name)
				if prev and prev.import then
					return prev
				end
				if not prev then
					error("Not new but not prev? " .. name .. " (prev parent: " .. parent.name .. ")", 3)
				end
				error("already non-imported version while trying to import " .. name .. " (prev parent: " .. parent.name .. ")", 3)
				return
			end
			local instance = class.new(name, parent)
			p.container.addChild(parent, instance)
			instance.external = true
			instance.import = importWksName		-- This contains either the imported workspace name or true
			return instance
		end
		-- AUDIOKINETIC End
...
---
	...
	function api._hasContainer(class, name)
		...
		if class.parent then
			...
			return p.container.getChild(parent, class, name) ~= nil
		end
		return true
	end
	-- AUDIOKINETIC End
