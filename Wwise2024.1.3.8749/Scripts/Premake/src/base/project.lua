...
---
		...
		-- AUDIOKINETIC mdonais Allowing to specifically import project as Workspace:Project
	function project.new(name, parent)
		local origName = name
		local importWksName = string.explode(name, ":")
		if #importWksName > 1 then
			name = importWksName[2]
		end

		local prj = p.container.new(project, name)
		prj.uuid = os.uuid(name)
		p.global.exportproject(parent, prj)	-- AUDIOKINETIC mdonais Adding importproject
...
--
	...
	function project.eachconfig(prj)
		...
		if prj._cfglist == nil then
			local wks = p.oven.bakeWorkspace(prj.parent)
			prj = p.workspace.findproject(wks, prj.name)
		end
		-- AUDIOKINETIC End
...
---
	...
	function project.getdependencies(prj, mode, wks) -- AUDIOKINETIC pmilot Add wks parameter
		...
		if wks == nil and result then -- AUDIOKINETIC pmilot check wks as well
			return result
		end

			local function add_to_project_list(cfg, depproj, result)
				local lookinwks = wks or cfg.workspace -- AUDIOKINETIC pmilot Get dependencies related to a specific solution
				local dep = p.workspace.findproject(lookinwks, depproj)
				dep = p.global.getexportedproject(dep)	-- AUDIOKINETIC mdonais Adding importproject
...
--
	...
	function project.mapconfig(prj, buildcfg, platform)
		...
		-- AUDIOKINETIC added workspace-based config maps
		maps = prj.wconfigmap or {}
		for mi = 1, #maps do
			for pattern, replacements in pairs(maps[mi]) do
				local prjname = replacements[#replacements]	-- Pattern for project selection: {"Debug", "MyProjectName"} or {"Debug", "x64", "MyProjectName"}
				if prjname == prj.name then
					if type(pattern) ~= "table" then
						pattern = { pattern }
					end
					...
					for i = 1, #pairing do
						if testpattern(pattern, pairing, i) then
							if #pattern == 1 and #replacements == 2 then
								pairing[i] = replacements[1]
							else
								pairing = { replacements[1], replacements[2] }
							end
