...
--
	...
	-- AUDIOKINETIC mdonais make sure a file is only generated once in our lifetime. Keep stats to show at premake close.
	premake.generatedFiles = {
		list = {},
		nochange = 0,
		updated = 0,
		new = 0
	...
	function premake.generate(obj, ext, callback)
		...
		-- AUDIOKINETIC mdonais make sure a file is only generated once in our lifetime
		if p.generatedFiles.list[fn] then
			warnf("File %s got generated twice!", fn)
		else
			p.generatedFiles.list[fn] = true
		end
		-- AUDIOKINETIC End
		...
		-- AUDIOKINETIC mdonais show all generated files, even if skipped, and tell which are new or updated. Use launch working directory for relative folders
		local fileexists = os.isfile(fn)
		local f, err = os.writefile_ifnotequal(output, fn);

		if (f == 0) then
			printf("Generated %s...", path.getrelative(_WORKING_DIR, fn))
			p.generatedFiles.nochange = p.generatedFiles.nochange + 1
		...
		elseif (f > 0) then
			if fileexists then
				...
				p.generatedFiles.updated = p.generatedFiles.updated + 1
			else
				printf("Generated %s... [New]", path.getrelative(_WORKING_DIR, fn))
				p.generatedFiles.new = p.generatedFiles.new + 1
			end
