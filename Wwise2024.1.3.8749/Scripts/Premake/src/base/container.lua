...
---
	...
	function container.bake(self)
		if self._isBaked then
			return self._isBaked		-- AUDIOKINETIC fix: Always return the baked version when baking
		end

		local ctx = p.context.new(self)
		self._isBaked = ctx		-- AUDIOKINETIC fix: Always return the baked version when baking
		ctx._isBaked = ctx		-- AUDIOKINETIC fix: Always return the baked version when baking
